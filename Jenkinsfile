...
pipeline {
    ...
    stages {
        ...
        // Nota SRE: la infraestructura de red/datos/cluster vive en infra-aws
        // (Terraform/networking-core, data-persistence, compute-edge) con su
        // propio ciclo de vida y pipeline. Este repo solo publica la imagen y
        // actualiza el Deployment/Service de la app, nunca el cluster ni el ALB.
        stage('Deploy to EKS') {
            when {
                expression { isMainBranch() }
            }
